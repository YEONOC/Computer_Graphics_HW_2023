...
void key_callback(GLFWwindow* window, int key, int scancode, int action, int mods)
{
  ...
  if (key == GLFW_KEY_P && action == GLFW_PRESS) {
      ...
      if (file.is_open()) {
          // camera count
          file << g_objects.size() << std::endl;
          for (std::size_t i = 0; i < g_objects.size(); ++i)
          {
              // filename
              file << g_objects[i].get_name() << std::endl;
              // scale
              file << g_objects[i].get_scale()[0] << " " << g_objects[i].get_scale()[1] << " " << g_objects[i].get_scale()[2] << std::endl;
              // translate
              file << g_objects[i].get_translate()[0] << " " << g_objects[i].get_translate()[1] << " " << g_objects[i].get_translate()[2] << std::endl;
          }
          // camera count
          file << g_cameras.size() << std::endl;
          for (std::size_t i = 0; i < g_cameras.size(); ++i)
          {
              // position
              file << g_cameras[i].position()[0] << " " << g_cameras[i].position()[1] << " " << g_cameras[i].position()[2] << std::endl;
              // at_position
              glm::vec3 at_position = g_cameras[i].position() - g_cameras[i].front_direction();
              file << at_position[0] << " " << at_position[1] << " " << at_position[2] << std::endl;
              // up_position
              glm::vec3 up_position = glm::normalize(g_cameras[i].up_direction());
              file << up_position[0] << " " << up_position[1] << " " << up_position[2] << std::endl;
          }
          file.close();
      }
