...
void scroll_callback(GLFWwindow* window, double x, double y)
{
  ...
  if (g_camera.mode() == Camera::kPerspective) {
    if (fovy >= 1.0f && fovy <= 179.0f)
      g_camera.set_fovy(fovy-y);
    if (fovy <= 1.0f) 
      g_camera.set_fovy(1.0f-y);
    if (fovy >= 179.0f)
      g_camera.set_fovy(179.0f+y);
  } 
  else {
    if (ortho_scale >= 0.1f && ortho_scale <= 10.0f)
      g_camera.set_ortho_scale(ortho_scale - y);
    if (ortho_scale <= 0.1f)
      g_camera.set_ortho_scale(0.1f-y*0.1f);
    if (ortho_scale >= 10.0f)
      g_camera.set_ortho_scale(10.0f-y);
  }
}
